#!/usr/bin/env python3
"""Convert Traditional Chinese to Simplified for RecommendedQuestInfoList_True.lua fields.

高效版本：单次遍历完成所有转换
处理字段：Title, Summary, QuestInfo1, QuestInfo2, QuestInfo3
"""
import re
import argparse
...
def convert_lua_file(text: str, converter: OpenCC):
    ...
    # 匹配目标字段
    pattern = re.compile(
        r'(Title|Summary|QuestInfo1|QuestInfo2|QuestInfo3)\s*=\s*',
        re.MULTILINE
    )
...
def main():
    parser = argparse.ArgumentParser(description='繁体转简体 - RecommendedQuestInfoList_True.lua')
