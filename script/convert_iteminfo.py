#!/usr/bin/env python3
"""Convert Traditional Chinese to Simplified for specific Lua fields.
...
def convert_lua_file(text: str, converter: OpenCC):
    ...
    # 匹配4个目标字段（包括单行和表）
    pattern = re.compile(
        r'(unidentifiedDisplayName|unidentifiedDescriptionName|'
        r'identifiedDisplayName|identifiedDescriptionName)\s*=\s*',
        re.MULTILINE
    )
...
def main():
    parser = argparse.ArgumentParser(description='繁体转简体 - 高效版本')
